...
// Send a message and get AI response - WebSocket streaming first, REST fallback
const sendMessage = async (req, res) => {
    try {
        const { message, sender, chat_id, agent_id, requestAudio, socketId } = req.body;
        ...
        if (socketId) {
            ...
            if (wsService && wsService.io) {
                const socket = wsService.io.sockets.sockets.get(socketId);
                
                if (socket) {
                    console.log('🔄 Delegating to WebSocket streaming service');
                    
                    // Delegate to WebSocket service for streaming
                    await wsService.handleChatMessage(socket, {
                        query: message,
                        agentId: agent_id,
                        leadId: null, // Will be extracted from session
                        voiceEnabled: requestAudio,
                        sessionId: chat_id,
                        sender: sender
                    });

                    // Return minimal REST response for WebSocket requests
                    return res.json({
                        success: true,
                        type: "websocket",
                        message: "Response being streamed via WebSocket",
                        data: {
                            socketId: socketId,
                            streaming: true,
                    ...
                    console.warn('⚠️ Socket not found, falling back to REST');
                }
            } else {
                console.warn('⚠️ WebSocket service not available, falling back to REST');
            }
        }

        // **REST Fallback Path**
        console.log('📡 Processing via REST API (no WebSocket available)');
        ...
        // Generate audio if requested (REST only)
        let audioData = null;
        if (requestAudio && elevenLabsService.isConfigured()) {
            try {
                console.log("🎵 Generating TTS audio for REST response");
                const audioBuffer = await elevenLabsService.textToSpeech(fullResponse);
        ...
        res.json({
            ...
            data: {
                ...
                audio: audioData,
                tokensUsed: {
                    ai: aiResponse.tokenUsage?.totalTokens || Math.ceil(fullResponse.length / 4),
